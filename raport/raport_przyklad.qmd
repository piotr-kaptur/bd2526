...
#| fig-cap: "Rekordy w poszczególnych kategoriach"
#| echo: false

df_display = df_best.rename(columns={
    'nazwa_konkurencji': 'Dyscyplina',
    'imie': 'Imię Chomika',
    'rasa': 'Rasa',
    'najlepszy_czas': 'Czas (s)'
})

Markdown(df_display.to_markdown(index=False))
```
**Wnioski:**
Wszyscy rekordziści, niezależnie od konkurencji, osiągają czasy w okolicy 15-16 sekund. Chomiki rasy rumuńskiej dzierżą najwięcej, bo aż 3 z 5 rekordów.
...
```{python}
#| label: fig-career-peak-line
#| fig-cap: "Średni czas w przedziałach wiekowych"
#| echo: false

interval = 100
df_age['wiek_koszyk'] = (df_age['wiek_dni'] // interval) * interval

df_trends = df_age.groupby(['nazwa_konkurencji', 'wiek_koszyk'])['czas'].mean().reset_index()
df_trends = df_trends.sort_values('wiek_koszyk')

fig = go.Figure()

dyscypliny = df_trends['nazwa_konkurencji'].unique()

for dysc in dyscypliny:
    df_d = df_trends[df_trends['nazwa_konkurencji'] == dysc]
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=8),
            hovertemplate="Wiek: %{x}-%{customdata} dni<br>Średni czas: %{y:.2f}s<extra></extra>",
            customdata=df_d['wiek_koszyk'] + interval,
            visible=False
        )
    )
...
for i, dysc in enumerate(dyscypliny):
    ...
    button = dict(
        ...
        args=[{"visible": visibility},
              {"title": f"Średni czas vs Wiek: {dysc}"}]    
    )
    buttons.append(button)

fig.update_layout(
    updatemenus=[dict(active=0, buttons=buttons, x=1.15, y=1.15)],
    title=f"Średni czas vs Wiek (co 50 dni): {dyscypliny[0] if len(dyscypliny)>0 else ''}",
    xaxis_title="Przedział wiekowy (początek przedziału w dniach)",
...
* **Długowieczność:** Zarówno w konkurencjach wymagających eksplozywnej energii ("Sprint prosty"), jak i technicznych ("Slalom tunelowy"), wykresy  utrzymują płaski trend. W "Biegu z przeszkodami" oraz "Sprincie prostym" starsze chomiki osiągają najlepsze czasy.
...
df_final = pd.merge(df_perf, df_money, on='id_chomika', how='inner')
```
